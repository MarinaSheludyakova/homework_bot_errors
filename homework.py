...
def parse_status(homework):
    ...
    """
    if homework.get('homeworks') is None or homework.get('homeworks') == []:
        message = ('Ошибка: список "homeworks" отсутствует или пуст.')
        ...
        if (homework_name is None) or (homework_name == ''):
            message = ('Ошибка: в ответе API нт ключа "homework_name".')
            logg_error_or_critical(logging.error, message)
            raise TypeError
        else:
